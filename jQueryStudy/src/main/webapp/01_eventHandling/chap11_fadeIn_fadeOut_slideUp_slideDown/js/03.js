...
*/	
	...
	/*
	   선택자.slideDown(3000); 은 선택자에 해당되는 내용물을 숫자(단위는 밀리초) 밀리초 동안 내용물을 위에서 부터 아래로 나타내어라는 말이다.
	   3000은 3초 이다.
	*/
/*	
	$("div.personInfo:eq(0)").slideDown('fast');
    $("div.personInfo").eq(1).slideDown('normal');
    $("div.personInfo:eq(2)").slideDown('slow');
*/    
	/*   
...
//	$("div.personInfo > ul:last-child").css('border','solid 3px red');
    ...
    // 프로필더보기를 클릭했을때 감추어진 해당 인물의 학력, 나이 정보를 3초 동안 위에서 부터 아래로 서서히 보여라. 
//  $("div.personInfo > ul:nth-child(2) > li:last-child").css('border','solid 3px red');
    ...
    // 프로필감추기를 클릭했을때 보여졌던 해당 인물의 학력, 나이 정보를 3초동안 아래서 부터 위로 서서히 숨겨라.
    ...
    $("div.personInfo > ul:last-child > li:last-child").each(function(i, elmt){ 
    	...
    	$(elmt).click(function(){
    	 //	$(this).parent().slideUp(3000);
    		/*
    		   선택자.slideUp(3000); 은 선택자에 해당되는 내용물을 숫자(단위는 밀리초) 밀리초 동안 아래서 부터 위로 보이지말라는 말이다.
	           3000은 3초 이다.
    		*/
